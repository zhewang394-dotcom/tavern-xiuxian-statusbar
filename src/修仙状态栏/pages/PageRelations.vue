<template>
  <section class="xy-page xy-page-relations">
    <div class="xy-npc-list">
      <!-- 主角 (本尊) 专属人物卡 -->
      <article
        class="xy-npc xy-protagonist-card"
        :class="{ 'xy-npc-open': state.openedNPC === '__user__' }"
        @click="state.openedNPC = state.openedNPC === '__user__' ? null : '__user__'"
      >
        <div class="xy-npc-head">
          <div
            class="xy-npc-avatar active"
          ...
          >
            ...
            />
            <span v-else class="xy-npc-avatar-char">{{ avatarChar(store.data.姓名) }}</span>
            <button
              type="button"
          ...
          <div class="xy-npc-info">
            <div class="xy-npc-line1">
              <span class="xy-npc-name" :class="`xy-npc-name--${npcGender(store.data)}`">{{ store.data.姓名 }}</span>
              <span class="xy-npc-realm">{{ store.data.修炼进度?.境界 || '凡人' }}</span>
              <span class="xy-npc-online">本尊</span>
            </div>
            <div class="xy-npc-line2">
              <span class="xy-npc-race">{{ store.data.种族 }}</span>
              <span v-if="(store.data.身份 && store.data.身份.length) || state.editMode">·</span>
              <IdentityTags v-model="store.data.身份" label="身份" />
              ...
              <span
                ...
                @click.stop="state.editMode && cycleEssence(store.data, '元阳')"
              ...
              <span
                ...
                @click.stop="state.editMode && cycleEssence(store.data, '元阴')"
              >元阴<span class="xy-bool-mark">{{ essenceMark(store.data.体质?.元阴) }}</span></span>
            </div>
          </div>
        </div>

        <div v-if="state.openedNPC === '__user__'" class="xy-npc-body" @click.stop>
          <!-- 基础信息四宫 -->
          <div class="xy-npc-grid">
            <div class="xy-mini">
              <h4>寿元</h4>
              <p>
                <EditableValue v-if="store.data.寿元" v-model.number="store.data.寿元.年龄" type="number" label="年龄" :min="0" /> /
                <EditableValue v-if="store.data.寿元" v-model.number="store.data.寿元.寿命" type="number" label="寿命" :min="1" />
                <small v-if="store.data.寿元">貌 <EditableValue v-model.number="store.data.寿元.外观年龄" type="number" label="外观年龄" :min="0" /></small>
              </p>
            </div>
            <div class="xy-mini">
              <h4>灵根</h4>
              <p>
                <span><EditableValue v-if="store.data.灵根" v-model="store.data.灵根.名称" label="灵根名称" :format="(v) => v || '未检测'" /></span>
                <span v-for="el in store.data.灵根?.五行 || []" :key="el" class="xy-element xy-element-mini" :style="{ '--el': elColor(el) }">{{ el === '未知' ? '未' : el === '混沌' ? '混' : el }}</span>
              </p>
            </div>
            <div class="xy-mini">
              <h4>体质</h4>
              <p>
                <EditableValue v-if="store.data.体质" v-model="store.data.体质.名称" label="体质名称" />
                <small v-if="store.data.体质">
                  悟<EditableValue v-model.number="store.data.体质.悟性" type="number" label="悟性" :min="0" />/骨<EditableValue v-model.number="store.data.体质.根骨" type="number" label="根骨" :min="0" />/感<EditableValue v-model.number="store.data.体质.气感" type="number" label="气感" :min="0" />
                </small>
              </p>
            </div>
            <div class="xy-mini">
              <h4>境界</h4>
              <p>
                <EditableValue v-if="store.data.修炼进度" v-model="store.data.修炼进度.境界" label="境界" />
                <small v-if="store.data.修炼进度">
                  <EditableValue v-model.number="store.data.修炼进度.当前进度" type="number" label="当前进度" :min="0" />/<EditableValue v-model.number="store.data.修炼进度.进度上限" type="number" label="进度上限" :min="1" />
                </small>
              </p>
            </div>
          </div>

          <div v-if="store.data.气质 || state.editMode" class="xy-mini-block">
            <h4>气质</h4>
            <p><EditableValue v-model="store.data.气质" label="气质" multiline :rows="3" /></p>
          </div>
          <div v-if="store.data.外貌 || state.editMode" class="xy-mini-block">
            <h4>外貌</h4>
            <p><EditableValue v-model="store.data.外貌" label="外貌" multiline :rows="3" /></p>
          </div>
          <div v-if="store.data.着装 || state.editMode" class="xy-mini-block">
            <h4>着装</h4>
            <p><EditableValue v-model="store.data.着装" label="着装" multiline :rows="3" /></p>
          </div>
        </div>
      </article>
